...
if [[ -z "$HYTALE_SERVER_SESSION_TOKEN" ]]; then
        if [[ "${HYTALE_DISABLE_UPDATES}" == "1" ]]; then
                echo -e "Update check disabled (HYTALE_DISABLE_UPDATES=1)."
                # First install: download if server files don't exist yet
                if ! [[ -e version ]]; then
                        echo -e "No server files found, performing initial download..."
